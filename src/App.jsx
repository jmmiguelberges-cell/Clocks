      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:12}}>
        {stys.map((s,i) => <div key={s.id} className={`anim d${i+1}`} style={{borderRadius:12,overflow:'hidden',background:'var(--bg)',border:'1px solid var(--border)'}}>
          ...
          <div style={{padding:'12px 14px'}}>
            ...
            <div style={{fontSize:12,color:'var(--teal)',fontWeight:500,marginTop:2}}>{s.role_title}</div>
          </div>
        </div>)}
      </div>
    </div>}
  ...
  return <div style={{maxWidth:480,margin:'0 auto',minHeight:'100vh',background:'var(--white)'}}>
    ...
    <div style={{display:'flex',margin:'0 28px',background:'var(--bg)',borderRadius:10,padding:3,marginBottom:24}}>
      {[['login','Iniciar sesión'],['register','Crear cuenta']].map(([id,l]) =>
        <button key={id} onClick={() => {setM(id);setEr('')}} style={{flex:1,padding:'11px 0',fontFamily:'inherit',fontSize:14,fontWeight:600,background:m===id?'var(--white)':'transparent',color:m===id?'var(--text)':'var(--text3)',border:'none',borderRadius:8,cursor:'pointer',boxShadow:m===id?'var(--shadow)':'none'}}>{l}</button>
      )}
    </div>
    <div className="anim" style={{padding:'0 28px 40px'}}>
      ...
      <p style={{fontSize:13,color:'var(--text3)',textAlign:'center',marginTop:18}}>
        ...
        <button onClick={() => {setM(m==='login'?'register':'login');setEr('')}} style={{fontFamily:'inherit',fontSize:13,color:'var(--teal)',background:'none',border:'none',cursor:'pointer',fontWeight:600}}>{m==='login'?'Regístrate':'Inicia sesión'}</button>
      </p>
    </div>
  </div>
}
...
function SvcRow({s, sel, onClick, i}) {
  return <button onClick={onClick} className={`anim d${(i%5)+1}`} style={{
    display:'flex',alignItems:'flex-start',justifyContent:'space-between',width:'100%',
    padding:sel?'14px 12px':'14px 0', borderRadius:sel?10:0,
    background:sel?'var(--teal-bg)':'transparent',
    border:sel?'1.5px solid var(--teal)':'none', borderTop:sel?'none':'1px solid var(--border)',
...
function Booking({user, profile, svcs, stys, pre, onDone, onBack}) {
  ...
  useEffect(() => {
    if (profile?.favorite_stylist_id && stys.length) {
      const f = stys.find(s => s.id === profile.favorite_stylist_id)
      if (f) setSty(f)
    }
  }, [profile, stys])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const cl = date.getDay() === 6 ? '14:00' : '20:00'
      const all = gS('09:00', cl), dur = svc?.duration || 30
  ...
  const days = gMD(cY,cM), can = [!!svc, !!sty, !!(date&&time)][step]
  ...
  return <div style={{paddingBottom:110}}>
    ...
    {step === 2 && <div style={{background:'var(--white)',padding:20}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:2,marginBottom:4}}>
        {dayL.map(d => <div key={d} style={{textAlign:'center',fontSize:11,fontWeight:600,color:'var(--text3)',padding:'5px 0'}}>{d}</div>)}
      </div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:2}}>
        {days.map((d,i) => {
          if (!d) return <div key={'e'+i}/>
          const pa = isP(d), su = d.getDay()===0, di = pa||su, sl = date && toK(date)===toK(d)
          return <button key={toK(d)} onClick={di?undefined:() => {setDate(d);setTime(null)}} style={{display:'flex',alignItems:'center',justifyContent:'center',height:42,borderRadius:21,background:'transparent',border:sl?'2px solid var(--teal)':'2px solid transparent',cursor:di?'default':'pointer',opacity:di?0.25:1}}>
            <span style={{fontSize:13,fontWeight:isT(d)?700:400,color:sl?'var(--teal)':'var(--text)'}}>{d.getDate()}</span>
          </button>
        })}
      </div>
      ...
      {date && <div className="anim" style={{marginTop:20}}>
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
          {slots.map(h => { const sl = time===h; return <button key={h} onClick={() => setTime(h)} style={{padding:'11px 18px',fontSize:14,fontWeight:500,borderRadius:22,background:sl?'var(--teal-bg)':'var(--white)',color:sl?'var(--teal)':'var(--text)',border:sl?'1.5px solid var(--teal)':'1.5px solid var(--border2)',cursor:'pointer'}}>{h}</button> })}
        </div>}
      </div>}
      ...
      {tab === 'settings' && <div style={{display:'flex',flexDirection:'column',gap:14}}>
        ...
        <div style={{background:'var(--white)',borderRadius:12,border:'1px solid var(--border)',padding:16}}>
          ...
          {[['Nombre',profile?.full_name],['Email',user.email],['Teléfono',profile?.phone||'—']].map(([k,v]) =>
            <div key={k} style={{display:'flex',justifyContent:'space-between',padding:'9px 0',borderBottom:'1px solid var(--border)',fontSize:14}}><span style={{color:'var(--text3)'}}>{k}</span><span style={{fontWeight:500}}>{v}</span></div>
          )}
        </div>
      </div>}
    </div>
...
function Admin({user, onBack, onDataChanged}) {
  ...
  const [bD, setBD] = useState(toK(new Date()))
  const [bSt, setBSt] = useState('09:00')
  const [bE, setBE] = useState('10:00')
  const [bR, setBR] = useState('')

  // CRUD modals
  const [editSvc, setEditSvc] = useState(null) // null=closed, {}=new, {id,...}=edit
  const [editSty, setEditSty] = useState(null)
  const [delConfirm, setDelConfirm] = useState(null) // {type:'service'|'stylist', id, name}
  ...
  const loadDay = useCallback(async d => {
    ...
    // Load profiles for appointment user_ids (FIX: separate query to avoid FK issues)
  ...
  const cancelAppt = async id => { await supabase.from('appointments').update({status:'cancelled',cancelled_by:'admin'}).eq('id',id); loadDay(sd) }
  ...
  const saveSvc = async (data) => {
    if (data.id) {
      await supabase.from('services').update({name:data.name,description:data.description,duration:data.duration,price:data.price,category:data.category}).eq('id',data.id)
    } else {
      const maxOrder = sv.reduce((m,s) => Math.max(m, s.display_order||0), 0)
      await supabase.from('services').insert({...data, display_order: maxOrder+1, active: true})
    }
  ...
  const saveSty = async (data) => {
    if (data.id) {
      await supabase.from('stylists').update({name:data.name,username:data.username,role_title:data.role_title,photo_url:data.photo_url,active:data.active}).eq('id',data.id)
    } else {
      const maxOrder = st.reduce((m,s) => Math.max(m, s.display_order||0), 0)
      await supabase.from('stylists').insert({...data, display_order: maxOrder+1, active: true})
    }
  ...
  return <div style={{minHeight:'100vh'}}>
    ...
    <div style={{display:'flex',background:'var(--white)',borderBottom:'1px solid var(--border)',padding:'0 20px'}}>
      {[['cal','📅 Calendario'],['team','👤 Equipo'],['svc','✂️ Servicios']].map(([id,l]) =>
        <button key={id} onClick={() => setTab(id)} style={{padding:'13px 14px',fontFamily:'inherit',fontSize:13,fontWeight:500,background:'none',border:'none',cursor:'pointer',color:tab===id?'var(--teal)':'var(--text3)',borderBottom:tab===id?'2px solid var(--teal)':'2px solid transparent'}}>{l}</button>
      )}
    </div>
    ...
    <div style={{padding:20}}>
      ...
      {tab === 'cal' && <div>
        ...
        <div style={{background:'var(--white)',borderRadius:12,border:'1px solid var(--border)',padding:14,marginBottom:16}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:2}}>
            ...
            {days.map((d,i) => {
              ...
              return <button key={toK(d)} onClick={() => setSd(d)} style={{height:30,borderRadius:15,background:sl?'var(--teal)':'transparent',border:'none',cursor:'pointer',fontSize:11,fontWeight:sl||isT(d)?700:400,color:sl?'#fff':'var(--text)'}}>{d.getDate()}</button>
            })}
          </div>
        </div>
        ...
        {ap.sort((a,b) => a.appointment_time.localeCompare(b.appointment_time)).map(a => {
          const s = stMap[a.status] || stMap.confirmed
          ...
          return <div key={a.id} style={{display:'flex',alignItems:'center',gap:10,padding:14,background:'var(--white)',border:'1px solid var(--border)',borderRadius:10,marginBottom:8,opacity:a.status==='cancelled'?0.4:1,boxShadow:'var(--shadow)'}}>
            ...
            {a.status === 'confirmed' && <button onClick={() => cancelAppt(a.id)} style={{fontSize:11,color:'var(--red)',background:'var(--red-bg)',border:'1px solid rgba(229,57,53,0.12)',borderRadius:6,padding:'5px 8px',cursor:'pointer',fontFamily:'inherit'}}>✕</button>}
          </div>
        })}

        {/* Block modal */}
        {showBlock && <Modal onClose={() => setShowBlock(false)}>
      ...
      {tab === 'team' && <div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
          <h2 style={{fontSize:18,fontWeight:800}}>Equipo</h2>
      ...
      {tab === 'svc' && <div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
          <h2 style={{fontSize:18,fontWeight:800}}>Servicios</h2>
    ...
    {/* Edit Service Modal */}
    {editSvc && <ServiceModal data={editSvc} onSave={saveSvc} onClose={() => setEditSvc(null)}/>}

    {/* Edit Stylist Modal */}
    {editSty && <StylistModal data={editSty} onSave={saveSty} onClose={() => setEditSty(null)}/>}

    {/* Delete Confirm */}
    {delConfirm && <Modal>
      <h3 style={{fontSize:18,fontWeight:800,marginBottom:12}}>¿Eliminar {delConfirm.name}?</h3>
...
function ServiceModal({data, onSave, onClose}) {
  ...
  const [dur, setDur] = useState(data.duration || 30)
...
// Stylist edit modal
function StylistModal({data, onSave, onClose}) {
  const [name, setName] = useState(data.name || '')
  const [username, setUsername] = useState(data.username || '')
  const [role, setRole] = useState(data.role_title || 'Barbero')
  ...
  return <Modal>
    ...
    {photo && <div style={{marginBottom:14,borderRadius:8,overflow:'hidden',height:80,width:80,background:'var(--bg)'}}>
    ...
    <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:14}}>
      ...
      <button onClick={() => setActive(!active)} style={{width:44,height:24,borderRadius:12,position:'relative',cursor:'pointer',border:'none',background:active?'var(--teal)':'var(--border)',transition:'all .3s'}}>
...
function Done({bk, onR}) {
  return <div className="scale-in" style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'60px 28px',textAlign:'center',minHeight:'80vh'}}>
    <div style={{width:76,height:76,borderRadius:38,background:'var(--green-bg)',display:'flex',alignItems:'center',justifyContent:'center',marginBottom:24}}>
    ...
    <p style={{fontSize:14,color:'var(--text2)',lineHeight:1.7,maxWidth:300}}>
      <strong>{bk.service.name}</strong> con {bk.stylist.name}<br/>{fDF(bk.date)} a las <strong>{bk.time}h</strong>
    </p>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const {data:{subscription}} = supabase.auth.onAuthStateChange((_e, s) => {
      if (s?.user) { setUser(s.user); lP(s.user.id) }
