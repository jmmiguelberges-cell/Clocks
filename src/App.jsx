...
// ═══ SERVICE CARD (shared Landing + Booking) ══════════════════════════════════
    ...
    <div style={{background:'var(--white)',padding:'20px 20px 16px',borderBottom:'1px solid var(--border)'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
        <div>
          ...
          <p style={{fontSize:11,fontWeight:700,color:'var(--purple)',letterSpacing:2.5,textTransform:'uppercase',marginBottom:4}}>Estudio de Barbería</p>
          <p style={{fontSize:12,color:'var(--text3)'}}>Fuentes de Ebro · Zaragoza</p>
        </div>
...
function Auth({onLogin,onBack}) {
  ...
  return <div style={{maxWidth:480,margin:'0 auto',minHeight:'100vh',background:'var(--white)'}}>
    ...
    <div style={{padding:'36px 28px 24px',textAlign:'center'}}>
      ...
      <h1 style={{fontSize:24,fontWeight:900,marginBottom:4,letterSpacing:-1,color:'var(--text)'}}>Clocks Estudio</h1>
...
function Booking({user,profile,svcs,stys,pre,onDone,onBack}) {
  ...
  // Pre-cargar disponibilidad mensual cuando hay barbero seleccionado
  useEffect(()=>{
    if(!sty)return
    ;(async()=>{
  ...
  // Cargar slots cuando hay fecha+barbero
  useEffect(()=>{
  ...
  return <div style={{paddingBottom:110}}>
    ...
    {step===2&&<div style={{padding:'0 16px'}}>
      ...
      <div style={{background:'var(--white)',borderRadius:18,border:'1.5px solid var(--border)',padding:16,marginBottom:12,boxShadow:'var(--shadow)'}}>
        ...
        {/* Días con barras de disponibilidad */}
        <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:2}}>
          {days.map((d,i)=>{
            ...
            return <button key={dk} onClick={()=>!past&&!sun&&setDate(d)} disabled={past||sun} style={{
              ...
              {/* Barra disponibilidad estilo Booksy */}
              {avC&&!past&&!sun&&(
          ...
          <div key={a.id} className="anim" style={{padding:16,background:'var(--white)',border:'1.5px solid var(--border)',borderRadius:16,boxShadow:'var(--shadow)'}}>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:12}}>
              ...
              <div style={{textAlign:'right'}}>
                <div style={{fontSize:14,fontWeight:600,color:'var(--text)'}}>{fS(new Date(a.appointment_date))}</div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:8}}>{hist.map(a=>
          <div key={a.id} style={{display:'flex',alignItems:'center',gap:12,padding:14,background:'var(--white)',border:'1.5px solid var(--border)',borderRadius:14,opacity:a.status==='cancelled'?0.5:1,boxShadow:'var(--shadow)'}}>
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:12,color:'var(--text3)',marginTop:2}}>{a.stylists?.name} · {fS(new Date(a.appointment_date))}</div>
            </div>
  ...
  return <div style={{minHeight:'100vh'}}>
    ...
    <div style={{padding:'14px 20px',background:'var(--white)',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        ...
        <div>
          ...
          <div style={{fontSize:10,color:'var(--text3)',letterSpacing:0.3}}>Gestión de citas</div>
        </div>
      </div>
    ...
    <div style={{display:'flex',background:'var(--white)',borderBottom:'1px solid var(--border)',padding:'0 20px'}}>
      {[['cal','📅 Calendario'],['team','👤 Equipo'],['svc','✂️ Servicios']].map(([id,l])=>
        <button key={id} onClick={()=>setTab(id)} style={{padding:'13px 14px',fontFamily:'inherit',fontSize:13,fontWeight:600,background:'none',border:'none',cursor:'pointer',color:tab===id?'var(--purple)':'var(--text3)',borderBottom:tab===id?'2.5px solid var(--purple)':'2.5px solid transparent'}}>{l}</button>
      )}
    </div>
      ...
      {tab==='cal'&&<div>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
          <div>
            ...
            <p style={{fontSize:12,color:'var(--text3)',marginTop:2}}>{cf} cita{cf!==1?'s':''} confirmada{cf!==1?'s':''}</p>
          </div>
          <Bt small variant="secondary" onClick={()=>{setBD(toK(sd));setShowBlock(true)}}>🚫 Bloquear</Bt>
        </div>

        {/* Bloques */}
        {bl.map(b=><div key={b.id} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',background:'var(--red-bg)',border:'1px solid rgba(239,68,68,0.12)',borderRadius:12,marginBottom:8}}>
